...
struct Substitution : public FunctionPass {
  ...
  Substitution() : FunctionPass(ID) { initSubstitutionFunctions(); }

  Substitution(bool flag) : FunctionPass(ID), flag(flag) {
    initSubstitutionFunctions();
  }

  void initSubstitutionFunctions() {
